# Ganesh Kumar Verma

**Full Stack Developer · MERN Stack · Data Science & AI**

[![Portfolio](https://img.shields.io/badge/Portfolio-ganesh--verma--portfolio.vercel.app-0f172a?style=flat-square&logo=vercel&logoColor=white)](https://ganesh-verma-portfolio.vercel.app/)
[![LinkedIn](https://img.shields.io/badge/LinkedIn-Connect-0a66c2?style=flat-square&logo=linkedin&logoColor=white)](https://www.linkedin.com/in/ganesh-verma-4358aa352/)
[![GitHub](https://img.shields.io/badge/GitHub-GANESHVERMA730-181717?style=flat-square&logo=github&logoColor=white)](https://github.com/GANESHVERMA730)
[![Resume](https://img.shields.io/badge/Resume-Download-16a34a?style=flat-square&logo=adobeacrobatreader&logoColor=white)](https://github.com/GANESHVERMA730/Resume.git)

---

## About Me

I'm a BCA (Data Science & AI) student at Shri Ramswaroop Memorial University with hands-on experience building full-stack web applications and data-driven tools.  
I've completed 8 months of structured MERN + DSA training (Apna College Sigma 10 Batch).  
I also completed a web development internship at CodSoft.  
I ship real projects, write clean code, and am actively looking for internship or junior full-stack roles.

---

## Skills

**Frontend** &nbsp; HTML · CSS · JavaScript · React.js  
**Backend** &nbsp;&nbsp;&nbsp; Node.js · Express.js  
**Database** &nbsp;&nbsp; MongoDB · MySQL  
**Languages** &nbsp;Python · JavaScript · C++ · Java  
**Data Science** &nbsp;Pandas · Matplotlib · Power BI  
**Tools** &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Git · GitHub · REST APIs · Postman · VS Code

---

## Projects

### Full Stack / MERN

| Project | Stack | Live |
|---|---|---|
| **House Rental Platform** | MongoDB · Express · React · Node.js | [View Live](https://full-stack-house-rental-platform.onrender.com/listings) |
| **Task Manager App** | MERN · REST API | [View Live](https://task-manager-psi-ebon-27.vercel.app/) |
| **Spotify Clone UI** | HTML · CSS · JavaScript | [View Live](https://spotify-clone-sable-six.vercel.app/) |
| **Netflix Clone** | HTML · CSS · JavaScript | [View Live](https://moviehub-demo.vercel.app/) |
| **Real-time Weather App** | JavaScript · OpenWeather API | API-based project |
| **Simon Game** | JavaScript · DOM | [View Live](https://simon-game-project-brown.vercel.app/) |

### Data Science / AI

| Project | Stack | Live |
|---|---|---|
| **Crop Recommendation System** | Python · Scikit-learn · Streamlit | [View Live](https://crop-recommendation-system-130.streamlit.app/) |
| **COVID-19 Analytics Dashboard** | Python · Pandas · Matplotlib | In progress |

---

## Experience

**Web Development Intern** — CodSoft *(1 month)*  
Built and deployed frontend projects; worked with responsive design and JavaScript fundamentals.

## MERN Stack + DSA Training — Apna College Sigma 10 Batch (8 months)

- Completed the full-stack development curriculum  
- Currently working on DSA practice and project development

---

## Education

**BCA — Data Science & AI**  
Shri Ramswaroop Memorial University, Lucknow  
*2022 – Present*

---

## Achievements

- Participated in **Smart India Hackathon (SIH)** — Internal Round
- Participated in **IIT Ropar GenAI Hackathon**
- Independently built and deployed multiple full-stack and data science projects

---

## GitHub Activity

![GitHub Streak](https://streak-stats.demolab.com?user=GANESHVERMA730&hide_border=true&date_format=M%20j%5B%2C%20Y%5D)

---

## Contact

Open to internship and junior full-stack developer roles.

📧 [bibekverma21@gmail.com](mailto:bibekverma21@gmail.com)  
💼 [linkedin.com/in/ganesh-verma-4358aa352](https://www.linkedin.com/in/ganesh-verma-4358aa352/)  
🌐 [ganesh-verma-portfolio.vercel.app](https://ganesh-verma-portfolio.vercel.app/)

---

*If you're hiring — I'd love to hear from you.*
